...
export default {
	...
	execute({ interaction, player, lang }) {
		...
		let embed = SimpleEmbed(GENERIC_ERROR[lang], EmbedType.Error);
		switch ((interaction.options as any)._hoistedOptions.map((x: any) => x.value).toString()) {
			case 'enable_loop_song':
				const success = queue.setRepeatMode(QueueRepeatMode.TRACK);
				if (success) embed = SimpleEmbed(`${LOOP_TRACK_EMBED_SUCCESS[lang]} ${queue.current.title}`, EmbedType.Success);
				break;

			case 'enable_loop_queue': {
				const success = queue.setRepeatMode(QueueRepeatMode.QUEUE);
				if (success) embed = SimpleEmbed(LOOP_QUEUE_EMBED_SUCCESS[lang], EmbedType.Success);
				break;
			}
			case 'disable_loop': {
				const success = queue.setRepeatMode(QueueRepeatMode.OFF);
				if (success) embed = SimpleEmbed(LOOP_OFF_EMBED_SUCCESS[lang], EmbedType.Success);
				break;
			}
			default:
				embed = SimpleEmbed(GENERIC_ERROR[lang], EmbedType.Error);
				break;
		}
		return interaction.reply({
			embeds: [embed],
