...
func init() {
	...
	if err != nil {
		log.Fatalf("[FATAL] Failed to start Discord session! Error: %v", err)
	}
}
...
func main() {
	session.AddHandler(func(session *discordgo.Session, ready *discordgo.Ready) {
		log.Printf("[INIT] Logged in as %v (%v)!", ready.User.String(), ready.User.ID)
		log.Printf("[INIT] Ready to serve %v guilds!", len(ready.Guilds))
	})

	err := session.Open()

	if err != nil {
		log.Fatalf("[FATAL] Failed to open Discord session! Error: %v", err)
	}
	...
	for i, v := range cmds {
		...
		if err != nil {
			log.Fatalf("[ERROR] Failed to register command %v! Error: %v", v.Name, err)
		}
		regCmds[i] = cmd
	}

	log.Println("[INIT COMPLETE] Successfully registered all commands!")
